...
#Get vote rank
def get_rank(session,post_data):
    try:
...
def vote_sina(session,id,disp):
    params['id'] = id
    timestamp = int(datetime.now().timestamp()*1000) #or time.time()
...
if __name__ == '__main__':
    post_data = "ids="
    for award in awards:
        post_data = "{}{},".format(post_data,award['id'])
    post_data = "{}&appid=yinhangzh19&page=1&pagesize=194".format(post_data[:-1])
    session = requests.session()
    get_rank(session,post_data)
